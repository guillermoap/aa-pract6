...
def play_game(dummy=True, weights=None, matches=0, train=True):
    board = Board(weights)
    ...
    if not moves < MAX_PLAYS:
        return TIE, green_player.weights, match_errors
    elif color == GREEN:
        return WIN, green_player.weights, match_errors
    elif color == RED:
        return LOSS, green_player.weights, match_errors
...
def training():
    ...
    while matches < MAX_MATCHES:
        result, weights, match_errors = play_game(weights=[weights, None], matches=matches)
        matches += 1
    ...
    print('####### RANDOM #######')
    print(f'FINAL WEIGHTS: {first_weights}')
...
def play():
    ...
    while matches < 100:
        result, _, _ = play_game(weights=[random_weights, last_weights], matches=matches, train=False)
        matches += 1
